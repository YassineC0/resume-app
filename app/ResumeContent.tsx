...
import { ChevronDown, Instagram, Facebook, Linkedin, Lightbulb, Palette, Twitter, Download, Check, Globe, Music, Waves, ExternalLink, X, Mail, Code, Dumbbell, Github } from 'lucide-react'
...
const LanguageSkill = ({ language, level }: { language: string, level: number }) => {
  const controls = useAnimation();
  const [ref, inView] = useInView({
    triggerOnce: false,
    threshold: 0.1,
      ...
      <div className="flex space-x-1">
        {[...Array(9)].map((_, i) => (
          <motion.div
            ...
            animate={controls}
            initial={{ scale: 0.5 }}
            className={`w-2 h-2 rounded-full ${i < level ? 'bg-green-400' : 'bg-gray-700'}`}
          />
        ))}
      </div>
    </div>
...
const InterestItem = ({ interest }: { interest: { icon: any, title: string, description: string } }) => {
  return (
    <motion.div
      className="flex flex-col items-center text-center"
      whileHover={{ scale: 1.05, boxShadow: "0px 0px 8px rgba(74, 222, 128, 0.5)" }}
...
const InsightItem = ({ children }: { children: React.ReactNode }) => {
  return (
    <motion.li
...
export default function ResumeContent() {
  ...
  let EducationList = [
    { year: '2023 - 2024 (Present)', degree: 'Software Engineer Student', school: 'International Academy Mohammed VI of Civil Aviation, AIAC' },
    { year: '2020 - 2022', degree: 'Preparatory Class', school: 'CPGE First Prepa, Meknes' },
    { year: '2017 - 2020', degree: 'Baccalaureate', school: 'Iqrae II School, Errachidia' },
  ]
  const projects = [
    {
  ...
  return (
    ...
    <div key={key} className="min-h-screen bg-black text-white">
      ...
      <main className="relative">
        <section className="relative h-screen flex flex-col justify-center items-center overflow-hidden">
          ...
          <div className="absolute bottom-4 left-4 z-10">
            <p>E: ghanmouni.yassine@aiac.ma</p>
            <p>T: +212 600 767 422</p>
          </div>
        </section>
        ...
        <div ref={introRef} className="container mx-auto px-4 py-16">
            ...
            <div className="bg-black bg-opacity-50 backdrop-blur-md rounded-lg p-8 max-w-4xl w-full relative z-10">
              <div className="flex flex-col lg:flex-row items-center lg:items-start">
                ...
                <div className="text-center lg:text-left">
                  ...
                  <Link
                    href="/Yassine-GHANMOUNI-CV.pdf"
                    download="Yassine-GHANMOUNI-CV.pdf"
                    target="_blank"
              ...
              <div className="md:w-1/2">
                ...
                <div className="space-y-2">
                  {DesignSkillsList.map((skill, index) => (
                    <SkillBar key={index} skill={skill} percentage={[90,90,80,66,70,60][index]} />
                  ))}
                </div>
              </div>
              <div className="md:w-1/2">
                ...
                <div className="space-y-4">
                  {LanguageList.map((language, index) => (
                    <LanguageSkill key={index} language={language} level={[9, 7, 5, 8][index]} />
                  ))}
                </div>
              </div>
            </div>
          ...
          <Section id="additional-insights">
            ...
            <ul className="list-disc list-inside space-y-4">
              <InsightItem>
                <strong>Problem-solving prowess:</strong> Skilled at breaking down complex issues into manageable components and developing innovative solutions.
              </InsightItem>
              <InsightItem>
                <strong>Continuous learning mindset:</strong> Eagerness to stay updated with the latest technologies and best practices in the ever-evolving field of software development.
              </InsightItem>
              <InsightItem>
                <strong>Collaborative spirit:</strong> Excellent communication skills and the ability to work effectively in cross-functional teams, contributing to a positive work environment.
              </InsightItem>
            </ul>
          </Section>
        </div>
      ...
      <div className="fixed top-0 right-0 h-full flex flex-col justify-center items-center space-y-4 p-4 bg-gray-900 bg-opacity-20 backdrop-blur-sm">
        <Link
          href="https://www.facebook.com/Yassine.Ghanmouni"
        ...
        <Link
          href="https://www.linkedin.com/in/yassine-ghanmouni-110444221/"
          className="text-white hover:text-green-400 transition-colors duration-300"
          target="_blank"
          rel="noopener noreferrer"
        >
        ...
        <Link
          href="https://github.com/YassineC0"
          className="text-white hover:text-green-400 transition-colors duration-300"
          target="_blank"
          rel="noopener noreferrer"
        >
        ...
        <Link
          href="mailto:ghanmouni.yassine@aiac.ma"
          className="text-white hover:text-green-400 transition-colors duration-300"
          target="_blank"
        >
          <Mail size={24} />
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-center">
            ...
            <div className="flex space-x-4">
              <Link
                href="https://www.facebook.com/profile.php?id=61550698631652&mibextid=LQQJ4d"
                className="text-white hover:text-green-400 transition-colors duration-300"
                target="_blank"
                rel="noopener noreferrer"
              >
